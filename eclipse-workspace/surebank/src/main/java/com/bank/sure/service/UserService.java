...
public class UserService {
	...
	public void register(RegisterRequest registerRequest) {
		...
		if(userRepository.existsByEmail(registerRequest.getEmail()) ) {
			throw new ConflictException(String.format(ExceptionMessage.EMAIL_ALREADY_EXISTS_MESSAGE, registerRequest.getEmail()));
		}
		
		if(userRepository.existsByEmail(registerRequest.getSsn()) ) {
			throw new ConflictException(String.format(ExceptionMessage.SSN_ALREADY_EXISTS_MESSAGE, registerRequest.getSsn()));
		}
		
		Set <Role> userRoles = new HashSet<>();
		//We add ROLE_CUSTOMER for ecery registered user by default 
		Role role = roleRepository.findByName(UserRole.ROLE_CUSTOMER).
				orElseThrow(()-> new ResourceNotFoundException(String.format(ExceptionMessage.ROLE_NOT_EXISTS_MESSAGE, UserRole.ROLE_CUSTOMER.name())));
